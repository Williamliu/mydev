...
$public_user["created_time"]= $row_public_user["created_time"];
...
if( $menuKey == "" ) { 
	$query_menu 	= "SELECT id FROM user_menu WHERE template like '%" . $temp_name . "';";
} else {
	$query_menu 	= "SELECT id FROM user_menu WHERE menu_key = '" . $menuKey . "';";
}
$result_menu 	= $db->query($query_menu);
if( $db->row_nums($result_menu) > 0 ) {
	$row_menu 							= $db->fetch($result_menu);
	$public_user["user"]["top_menuid"] 	= $row_menu["id"];
} else {
	if( $menuKey == "" ) { 
		$query_menu 	= "SELECT a.id FROM user_menu a INNER JOIN user_template b ON (a.id = b.ref_id) WHERE b.template like '%" . $temp_name . "';";
	} else {
		$query_menu 	= "SELECT a.id FROM user_menu a INNER JOIN user_template b ON (a.id = b.ref_id) WHERE b.menu_key = '" . $menuKey . "';";
	}
	$result_menu 						= $db->query($query_menu);
	$row_menu 							= $db->fetch($result_menu);
	$public_user["user"]["top_menuid"] 	= $row_menu["id"];
...
<script type="text/javascript" 	src="<?php echo $CFG["web_domain"]?>/jquery/myplugin/jquery.lwh.accord.js"></script>
<link 	type="text/css" 	    href="<?php echo $CFG["web_domain"]?>/jquery/myplugin/css/light/jquery.lwh.accord.css" rel="stylesheet" />
