<script type="text/javascript" language="javascript">
	...
	$(function(){
		...
		$("[name=user_name]", ".top-header-login").bind("keydown", function(ev) {
			if (ev.keyCode == 13) {
				$("[name=password]", ".top-header-login").focus();
			}
		});
	});
	...
	<div class="top-header">
    	<div class="frame-center">
            <div class="top-header-navi">
				...
				<a class="contact"  id="btn-publicContactus" href="javascript:void(0);" title="<?php echo $words["navi.contact"];?>"></a>
            ...
            <div class="top-header-login">
                ...
                <?php } else { ?>
                    ...
                    <input type="text" scope="userlogin" coltype="textbox" 		name="user_name" need="1" notnull="1" style="width:110px; font-size:12px;" colname="<?php echo $words["name.email.phone"]?>" placeholder="<?php echo $words["name.email.phone"] ?>" value="" />
                    <span class="label-text" style="margin-left:10px;"><?php echo $words["password"] ?>: </span>
                    <input type="password" scope="userlogin" coltype="textbox" 	name="password"   need="1" notnull="1" minlength="6" colname="<?php echo $words["password"]?>" style="width:70px; font-size:12px;" placeholder="" value="" />
        
                    <input class="button btn-signin" scope="userlogin" coltype="save" type="button" value="<?php echo $words["login"] ?>" />
                    <input class="button btn-register" id="btn-publicRegister"	type="button"  value="<?php echo $words["register"] ?>" />
