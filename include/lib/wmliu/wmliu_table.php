...
class WMLIUTABLE {
	...
	public 	$error;
	public 	$errorMessage;
	public function __construct($db, $table, $dlang, $user_right) {
		$this->table            = $table;
        $this->user				= $user_right;
		$this->vlist            = array();
        ...
        //if($ssccc=="") $this->table["navi"]["head"]["qmode"] = "view";
        if($ssccc=="" && $this->table["navi"]["head"]["match"]=="1") $this->table["navi"]["head"]["qmode"] = "view";

        switch($this->table["navi"]["head"]["qmode"]) {
            case "view":
                        // case of view
            ...
            case "search":
                ...
                if( $pid == "" )        $criteria   = WMSEARCH::concat($criteria, " 1 = 0 ");
                if($criteria=="" && $this->table["navi"]["head"]["match"]=="1") $criteria = WMSEARCH::concat($criteria, " 1 = 0 ");

                if($pptable != "" ) {
                        if($mmtable != "") {
                                if($ddtable!="") { 
                                        $this->query = "SELECT $pptable.$pcol as pid, $sstable.$scol as sid, $ddtable.$dcol as did,  
                                                                IF(ISNULL($pptable.$pcol), 0, 1) AS refcheck, 
                                                                            $field_str FROM $sstable 
                                                                                        LEFT JOIN $mmtable ON ( $sstable.$scol = $mmtable.$msref AND $mmtable.$mpref = '$pid' )
                                                                                        LEFT JOIN $pptable ON ( $mmtable.$mpref = $pptable.$pcol AND $pptable.$pcol = '$pid' ) 
                                                                                        LEFT JOIN ( SELECT aaa.* FROM $ddtable aaa WHERE aaa.deleted <> 1 ) $ddtable ON ( $sstable.$scol = $ddtable.$dcol )
                                                                            WHERE $sstable.deleted <> 1 $criteria";
                                } else {
                                        $this->query = "SELECT $pptable.$pcol as pid, $sstable.$scol as sid,  
                                ...
                                if($ddtable!="") {
                                    $this->query = "SELECT $pptable.$pcol as pid, $sstable.$scol as sid,  $ddtable.$dcol as did, 
                                                            IF(ISNULL($pptable.$pcol), 0, 1) AS refcheck, 
                                                                            $field_str FROM $sstable 
                                                                                    LEFT JOIN $pptable ON ( $pptable.$pcol = $sstable.$spref  AND $pptable.$pcol = '$pid' ) 
                                                                                    LEFT JOIN ( SELECT aaa.* FROM $ddtable aaa WHERE aaa.deleted <> 1 ) $ddtable ON ( $sstable.$scol = $ddtable.$dcol ) 
                                                                        WHERE $sstable.deleted <> 1 $criteria";
                                } else {
                                    $this->query = "SELECT $pptable.$pcol as pid, $sstable.$scol as sid, 
	...
	private function orderStr() {
		...
		} elseif($this->table["navi"]["head"]["orderBY"] != "") {
        ...
        while( $row = $db->fetch($result)) {
		    ...
		    foreach( $this->table["schema"]["cols"] as $col ) {
                if($col["col"]!="") {
			        switch($col["type"]) {
                        case "datetype":
                            $row[$col["start_date"]]    = $row[$col["start_date"]]>0?$row[$col["start_date"]]:"";
                            $row[$col["end_date"]]      = $row[$col["end_date"]]>0?$row[$col["end_date"]]:"";
                            $row[$col["once_date"]]     = $row[$col["once_date"]]>0?$row[$col["once_date"]]:"";
                            $row[$col["start_time"]]    = $row[$col["start_time"]]>0?$row[$col["start_time"]]:"";
                            $row[$col["end_time"]]      = $row[$col["end_time"]]>0?$row[$col["end_time"]]:"";

							switch( $row[$col["date_type"]]) {
								case "Daily":
									break;
								case "Weekly":
									$wdates = explode(",", $row[$col["date_sets"]]);
									$warr = array();
									foreach($wdates as $wval) {
										$warr[$wval] = true;
									}
									$rows[$cnt][$col["col"]]["wdates"] = $warr;
									break;
								case "Monthly":
									$wdates = explode(",", $row[$col["date_sets"]]);
									$warr = array();
                        ...
                        case "radiodiag":
                            ...
                            if($col["other"]!="") $rows[$cnt][$col["other"]] = $row[$col["other"]]?$row[$col["other"]]:"";
                            break;
			            ...
			            case "checkbox":   // { 2: true, 4: true, 5: true }
                        case "checkcom":
                        case "checklist":
                        case "checkdiag":
				            $ref_table 	= $this->table["schema"]["checklist"][$col["col"]]["rtable"];
				            $ref_col 	= $this->table["schema"]["checklist"][$col["col"]]["rcol"];

                            $col["table"] = $col["table"]!=""?$col["table"]:"sstable";
                            if($col["table"]=="ddtable")
                                $query_list = "SELECT $ref_table." . $col["col"] ." FROM $ref_table WHERE $ref_table.$ref_col = '" . $row["did"] . "'";  
                            else
                                // reference always use primary table primary key:  id
                                $query_list = "SELECT $ref_table." . $col["col"] ." FROM $ref_table WHERE $ref_table.$ref_col = '" . $row["sid"] . "'";  
    							//echo "query: " . $query_list;
                            
                            $result_list = $db->query($query_list);
                            $check_val = array();
                            while( $row_list = $db->fetch($result_list) ) {
                                $check_val[$row_list[$col["col"]]] = true;                                
                            }
                            $rows[$cnt][$col["col"]] = count($check_val)>0?$check_val:null;
                            if($col["other"]!="") $rows[$cnt][$col["other"]] = $row[$col["other"]]?$row[$col["other"]]:"";
                            break;
			            case "cktext": //  [2, 3, 5]
				            $ref_table 	= $this->table["schema"]["checklist"][$col["col"]]["rtable"];
                        ...
                        case "thumb":
							...
							break;
                        case "refcheck":  //  true / false
                        case "bool":  //  true / false
        				    $rows[$cnt][$col["col"]] = $row[$col["col"]]==1||$row[$col["col"]]=="1"?true:false;
                            break;
                        case "text": //  "abc"
        				    $rows[$cnt][$col["col"]] = LANG::trans($db->unquote($row[$col["col"]]), $this->table["navi"]["head"]["lang"]);
                            break;
                        case "ptext": //  "abc"
        				    $rows[$cnt][$col["col"]] = $this->table["schema"]["idvals"]["pid"];
                            break;
                        default:     // null => ""
        				    $rows[$cnt][$col["col"]] = $row[$col["col"]]?$db->unquote($row[$col["col"]]):"";
                            break;
			        }
                }
		    }
    ...
    public function save() {
	    ...
	    foreach($this->table["rows"] as $key=>$row) {
            ...
            $row["refcheck"] = ($row["refcheck"]==="true" || $row["refcheck"]===true)?true:false;
            $this->table["rows"][$key]["refcheck"] = $row["refcheck"]==true?true:false;
    ...
    private function getRow($row, $key, $tableLevel) {
		...
		foreach($this->table["schema"]["cols"] as $col) {
			if($col["type"]=="datetype") {
				$dddtype[$col["col"]]["date_type"] 	= $col["date_type"]; 
				$dddtype[$col["col"]]["start_date"] = $col["start_date"]; 
				$dddtype[$col["col"]]["end_date"] 	= $col["end_date"]; 
				$dddtype[$col["col"]]["start_time"] = $col["start_time"]; 
				$dddtype[$col["col"]]["end_time"] 	= $col["end_time"]; 
				$dddtype[$col["col"]]["once_date"] = $col["once_date"]; 
				$dddtype[$col["col"]]["date_sets"] 	= $col["date_sets"]; 
			}
		}
        ...
        foreach($this->table["schema"]["cols"] as $col) {
            ...
            switch($col["type"]) {
                ...
                case "number":
                    ...
                    if($len > 0 &&  isset($col["min"]) && isset($col["max"]) ) {
                    ...
                    if($len > 0 && $len < $col["minlength"] && $col["minlength"]>0) {
                           $this->table["rows"][$key]["error"] = 1;
				...
				case "seal":
					$dtObj = $dddtype[$col["ref"]];
					//$colName==$dtObj["date_type"];
					$datetype = $row[$dtObj["date_type"]];
					$ver_flag = false;
					$valtype  = "CHAR";
					if( in_array($colName, array($dtObj["date_type"])) ) {
						$ver_flag = true;
						$temp[$colName] = $row[$colName]?$row[$colName]:"";
					}
   					...
   					switch($datetype) {
						    ...
						    case "Weekly":
								...
								if( in_array($colName, array($dtObj["date_sets"])) ) {
									...
									$select_ck = false;
									$val_str   = "";
									foreach($row[$col["ref"]]["wdates"] as $key1=>$val1) {
										if(strtolower($val1)=="true"||$val1===true) {
											$select_ck = true;
											$val_str   .= ($val_str!=""?",":"") . $key1;
                                            // prevent true => "true"  ,  javasciprt "true" != true
                                            $this->table["rows"][$key][$col["ref"]]["wdates"][$key1] = true;
										}
									}
									// prevent "true"
                                    foreach($row[$col["ref"]]["mdates"] as $key1=>$val1) {
                                        $this->table["rows"][$key][$col["ref"]]["mdates"][$key1] = false;
                                    }

									if(!$select_ck) {
										$this->table["rows"][$key]["error"] = 1;
										$this->table["rows"][$key]["errorMessage"][$colName] .= ($this->table["rows"][$key]["errorMessage"][$colName]==""?"":"\n") .  "'" . ($col["title"]?$col["title"]:ucwords($colName)) . "' " . LANG::words("required_col", $lang);
										$this->table["rows"][$key]["errorMessage"][$col["ref"]] .= ($this->table["rows"][$key]["errorMessage"][$col["ref"]]==""?"":"\n") .  "'" . ($col["title"]?$col["title"]:ucwords($colName)) . "' " . LANG::words("required_col", $lang);
									}
								...
								if( in_array($colName, array($dtObj["date_sets"])) ) {
									...
									foreach($row[$col["ref"]]["mdates"] as $key1=>$val1) {
										if(strtolower($val1)=="true"||$val1===true) {
											...
											$val_str   .= ($val_str!=""?",":"") . $key1;
                                            // prevent true => "true"  ,  javasciprt "true" != true
                                            $this->table["rows"][$key][$col["ref"]]["mdates"][$key1] = true;
										}
									}
									// prevent "true"
                                    foreach($row[$col["ref"]]["wdates"] as $key1=>$val1) {
                                        $this->table["rows"][$key][$col["ref"]]["wdates"][$key1] = false;
                                    }
					...
					if($ver_flag) {
						...
						if($len > 0) {
							if(!preg_match($this->dataType[ strtoupper($valtype) ], $row[$colName])) {
								$this->table["rows"][$key]["error"] = 1;
								$this->table["rows"][$key]["errorMessage"][$colName] 	.= ($this->table["rows"][$key]["errorMessage"][$colName]==""?"": "\n") .  "'" . ($col["title"]?$col["title"]:ucwords($colName)) . "' " . LANG::words("invalid_type", $lang, strtoupper($valtype) );
								$this->table["rows"][$key]["errorMessage"][$col["ref"]] .= ($this->table["rows"][$key]["errorMessage"][$col["ref"]]==""?"": "\n") .  "'" . ($col["title"]?$col["title"]:ucwords($colName)) . "' " . LANG::words("invalid_type", $lang, strtoupper($valtype) );
							}
						}
					}
